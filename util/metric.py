...
def compute_depth_errors(pred, gt):
    ...
    """
    # np.log(1+output[img_type]) / ( np.log( 2. ** 16.0 ) )

    # global AA
    # print('before gt: ', gt.median(), gt.mean(), gt.min(), gt.max())
    # depth_single_image(gt[0:1].detach().cpu(), '/gpfs/u/home/AICD/AICDzich/scratch/new_small_gt_'+str(AA)+'.png')
    # mask = gt > 0
    ...
    gt = ( gt - 0.64 ) / 0.18
    gt = ( gt + 1. ) / 2
    gt = gt * 128.0 # map to [0,80]
    gt = torch.clamp(gt, min=1e-3, max=128.0)

    # print('pred: ', pred.median(), pred.mean(), pred.min(), pred.max())
    # depth_single_image(pred[0:1].detach().cpu(), '/gpfs/u/home/AICD/AICDzich/scratch/new_small_pred_'+str(AA)+'.png')
    ...
    # print('prepred: ', pred.mean(), pred.min(), pred.max())
    ...
    pred = ( pred + 1. ) / 2
    pred = pred * 128.0
    pred = torch.clamp(pred, min=1e-3, max=128.0)

    # print('gt: ', gt.median(), gt.mean(), gt.min(), gt.max())
    # print('pred: ', pred.mean(), pred.min(), pred.max())
