...
def init_distributed_mode(args):
    ...
    args.init_distributed_mode = True

    torch.cuda.set_device(args.gpu)
    args.dist_backend = 'nccl'  
    # | distributed init (rank 0): backend nccl, env://, gpu 0 worldsize 8
    # | distributed init (rank 0): backend nccl, file:///nobackup/users/zitian/experiments
    print('| distributed init (rank {}): backend {}, {}, gpu {} worldsize {}'.format(
        args.rank, args.dist_backend, args.dist_url, args.gpu, args.world_size), flush=True)
...
def Frozen_blk(model, in_moe):
    for blk in model.module.blocks:
        ...
        blk.mlp.output_experts.w.grad[:in_moe['mlp.output_experts.w']] = 0.0
...
class NativeScalerWithGradNormCount:
    ...
    def __call__(self, loss, optimizer, clip_grad=None, parameters=None, create_graph=False, update_grad=True, model=None, in_moe=None, frozen=False):
        self._scaler.scale(loss).backward(create_graph=create_graph)

        if model is not None and frozen:
            Frozen_blk(model, in_moe)
            # if random.randint(0,100) < 5:
            #     torch.set_printoptions(precision=10)
            #     print(model.module.blocks[11].attn.q_proj.experts.w)
            #     print(model.module.blocks[11].attn.q_proj.experts.w.shape)
        ...
        if is_main_process():
            ...
            for file in files:
                if file[:10] == 'checkpoint':
                    print('remove ', os.path.join(args.output_dir, file))
